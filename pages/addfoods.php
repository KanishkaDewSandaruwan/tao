...
<div class="col-md-12">
       <div class="row">
						<form class="col-md-10 ml-5" action="addfoods.php" method="POST" enctype="multipart/form-data">
                      <a href="addfoods.php"><h2 class="text-info">Add Food Items </h2> </br></a>
                                <?php 

                                  $q4 = "SELECT * FROM food_items WHERE food_id =(SELECT max(food_id ) FROM food_items);";
                                  $res4 = mysqli_query($con,$q4);
                                  $row2 = mysqli_fetch_assoc($res4);

                                  if (isset($row2['food_id'])) {
                                    $cat_id = $row2['food_id'];
                                    $last = substr(trim($cat_id), -1);
                                    $newCode = $last + 1;
                                    $new_last_code = "FOOD0".$newCode;
                                  }else{
                                    $new_last_code = "FOOD01";
                                  }
                                 ...
                                 ?>
                                  ...
                                  <div class="form-row">
                                      <div class="form-group col-md-6">
                                        <label for="code" class="a"><b>Food Code</b></label>
                                        <input type="text" class="form-control" style="text-transform: uppercase;" Value="<?php echo $new_last_code; ?>" name="get_code" placeholder="Food Code">
                                      </div>
                                    </div>
                                    ...
                                    <div class="form-row">
                                    <div class="form-group col-md-6">
                                      <label for="original_price"><b>Price</b></label>
                  ...
                  <div class="dropdown-divider"></div>
                   ...
                   <button type="button" name="submit" onclick="window.location.href='viewfooditem.php'" class="btn col-xl-10 btn-success" style="border-radius:20px;">Food List</button>
                   </div>
                   </div>
                                    <?php
                                                  ...
                                                  if(!empty($price)){
                                                          // if(!empty($image)){
                                                            ...
                                                            $cat_id = $row2['cat_id'];
                                                                    ...
                                                                    $query3="SELECT * FROM food_items WHERE  food_name='$title'";
                                                                    $sql3=mysqli_query($con,$query3);

                                                                    if(mysqli_num_rows($sql3)>0)
                                                                    {
                                                                      echo '<script>alert("This Food is Alrady Here.");</script>';
                                                                    }else{
                                                                      if (in_array($imageFileType,$extensions_arr)) {

                                                                        move_uploaded_file($_FILES['file']['tmp_name'],$target_dir.$name);

                                                                        $q1="INSERT INTO food_items(food_id,food_cat_id,food_name,description,image,available,trndate,price) values('$code','$cat_id','$title','$desc','$name','$available','$cdate','$price')";

                                                                            $res =  mysqli_query($con,$q1);
                                                                          if ($res) {
                                                                            echo "<script type=\"text/javascript\"> alert(\"Food Item Added Succussfully\"); window.location= \"addfoods.php\";</script>";
                                                                          }else{
                                                                            echo "<script>alert(\"Food Stock Adding is Not Scussess\");</script>";
                                                                          }
                                                            ...
                                                            // }else{ echo "<script>alert(\"Please Upload Image\");</script>";}
                                                      }else{ echo "<script>alert(\"Please Enter Food Price\");</script>";}
                                                    }else{ echo "<script>alert(\"Please Select Food Categories\");</script>";}
                                                }else{ echo "<script>alert(\"Please Enter Description\");</script>";}
                                              }else{echo "<script>alert(\"Please Enter Food Name\");</script>";}
                                        }
